package commands.leaderCommands.wrRaid;
 ...
 */
public class CommandWrStartRaid extends Command {
    public CommandWrStartRaid() {
        setAliases(new String[] {"wrraid", "wrnewraid"});
    ...
    public void execute(Message msg, String alias, String[] args) {
        ...
        if (args.length == 1) {
            try {
                String location = args[0];
                RaidHub.startRaid(msg.getMember(), 0, -1, location, RaidHub.RaidType.WR_RAID);
                return;
            } catch (Exception ex) {
                Utils.sendPM(msg.getAuthor(), "Invalid raid location! Please retry");
                return;
        ...
        if (args.length == 2) {
            try {
                String location = args[0];
                int size = Integer.parseInt(args[1]);
                RaidHub.startRaid(msg.getMember(), size, -1, location, RaidHub.RaidType.WR_RAID);
                return;
            } catch (Exception ex) {
                Utils.sendPM(msg.getAuthor(), "Invalid raid size or location! Please retry");
                return;
        ...
        if (args.length == 3) {
            try {
                ...
                int size = Integer.parseInt(args[1]);
                int time = Integer.parseInt(args[2]);
                RaidHub.startRaid(msg.getMember(), size, time, location, RaidHub.RaidType.WR_RAID);
                return;
            } catch (Exception ex) {
                Utils.sendPM(msg.getAuthor(), "Invalid raid size, timer or location! Please retry");
                return;
        ...
        RaidHub.startRaid(msg.getMember(), 0, -1, null, RaidHub.RaidType.WR_RAID);
    }
    ...
    public EmbedBuilder getInfo() {
        ...
        embedBuilder.setTitle("Command: Create world record raid");
